...
The four ERC-4626 actions also have exact adjacent-integer boundaries. If `q = previewDeposit(a)`, then `previewMint(q) <= a < previewMint(q + 1)`. If `a = previewMint(s)`, then `previewDeposit(a - 1) < s <= previewDeposit(a)`. If `s = previewWithdraw(a)`, then `previewRedeem(s - 1) < a <= previewRedeem(s)`. If `a = previewRedeem(s)`, then `previewWithdraw(a) <= s < previewWithdraw(a + 1)`. Four focused properties exercise those inequalities over 10,000 seeded-and-donated exchange-rate cases each, then execute the corresponding action and reconcile exact asset, share, `totalAssets()`, and `totalSupply()` deltas. Return-value equality alone would be weaker evidence because the inherited actions call their corresponding preview functions internally.
